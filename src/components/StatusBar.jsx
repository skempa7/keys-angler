...
export default function StatusBar() {
  ...
  const { applied, toggleTheme } = useTheme()
  return (
    <div className="row" style={{ gap: 'var(--sp-2)' }}>
      ...
      <button
        className="icon-btn"
        onClick={toggleTheme}
        aria-label={applied === 'sea' ? 'Switch to sunlight mode' : 'Switch to sea mode'}
        title={applied === 'sea' ? 'Sunlight (high-contrast) mode' : 'Sea (dark) mode'}
      >
        {applied === 'sea' ? <IcSun /> : <IcMoon />}
      </button>
    </div>
  )
}
